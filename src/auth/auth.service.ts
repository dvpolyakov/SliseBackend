...
export class AuthService {
  ...
  public async authUser(request: AuthUserRequest): Promise<JwtTokenModel> {
    try {
      ...
      if (!user) {
        ...
        const jwtTokenModel = this.createToken(createdUser);
        jwtTokenModel.whitelistId = wl.id;
        jwtTokenModel.publicLink = wl.publicLink;
        return jwtTokenModel;
      }
      // return user.address;
      const jwtTokenModelExist = this.createToken(user);
  ...
  public async authWhitelistMember(
    request: AuthWhitelistMember,
  ): Promise<string> {
    ...
    if (!existWhitelist) throw new BadRequestException('Whitelist not found');
    ...
    const whitelistMember = await this.prisma.whitelistMember.create({
      data: {
        address:
          request.networkType === NetworkType.Solana
            ? request.address
          ...
          create: {
            ...
            twitterFollowers:
              request.twitter === null
                ? null
                : await this.integrationService.getTwitterFollowersCount(
                    request.twitter,
                  ),
          },
        },
    ...
    this.logger.debug(
      `Saved whitelist member ${whitelistMember.address} in whitelist ${whitelistMember.whitelistId}`,
    );
  ...
  private createToken(userModel: User): JwtTokenModel {
    const exp = new Date();
